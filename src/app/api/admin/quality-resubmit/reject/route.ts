...
import { rejectAdminQualityResubmit } from "@/lib/admin-quality-resubmit";
...
export async function POST(req: Request) {
  ...
  let body: { listingId?: unknown; storageTable?: unknown; reason?: unknown };
  try {
  ...
  const result = await rejectAdminQualityResubmit(
    ctx.supabase,
    listingId,
    storageTable,
    reason
  ...
  if (!result.ok) {
    return NextResponse.json(
      { ok: false, error: result.error ?? "reject_failed" },
      { status: 409 }
    );
  }
